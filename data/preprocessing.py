...
def augment_point_cloud(point_cloud, jitter_sigma=0.01, rotate=True):
    ...
    # Add random jitter
    augmented_pc = augmented_pc + np.random.normal(0, jitter_sigma, size=augmented_pc.shape)
...
def prepare_data_for_training(pairs, augment=True):
    ...
    train_pairs, val_pairs = train_test_split(normalized_pairs, test_size=0.2, random_state=42)
